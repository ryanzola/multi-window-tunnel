...
void main() {
  // normalize uvs
  vec2 uv = rotate(vUv - 0.5, uLevel * PI);

  // float scene = sdCircle(uv, 0.2);
  // float scene = sdBox(uv, vec2(0.2));

  float nnn = cnoise(vec3(uv * 5.0, 3.0));

  float scene = mix(
    sdCircle(uv, 0.2),
    sdBox(uv, vec2(0.2)),
    1.0 * (sin(uLevel * PI * 2.0) + 1.0)
  );
  ...
  gl_FragColor = vec4(col + color * 0.6, 1.0);
  // gl_FragColor = vec4(color, 1.0);

  float dist = length(cameraPosition - vWorldPosition);
  float fade = smoothstep(14.0, 7.0, dist);
